...
public class LogFile {
	boolean doLogBinding = false, doLogParser = false,
			doLogPrettyPrint = false, doLogScanner = false,
	...
	/**
	 * Make a note in the log file that a source line has been read. This note
	 * is only made if the user has requested appropriate logging.
	 *
	 * @param lineNum
	 *            The line number
	 * @param line
	...
	/*
	 * Make a note in the log file that a token has been read. This note will
	 * only be made if the user has requested it.
	 */
	public void noteToken(Token tok) {
		if (doLogScanner)
			writeLogLine("Scanner: " + tok.identify());
		System.out.println("Scanner: " + tok.identify());
	}

	/*
	 * Del 3: public void noteTypeCheck(String op, Type t, PascalSyntax where) {
	 * if (doLogTypeChecks) writeLogLine("Type check on line " + where.lineNum +
	 * ": " + "{" + op + "} " + t.identify()); }
	 * 
	 * public void noteTypeCheck(Type t1, String op, Type t2, PascalSyntax
	 * where) { if (doLogTypeChecks) writeLogLine("Type check on line " +
	 * where.lineNum + ": " + t1.identify() + " {" + op + "} " + t2.identify());
	 * }
	 */
	...
	/**
	 * Make a note that the parser has startet parsing a non-terminal. This note
	 * is only recorded if the user has requested it.
	 *
	 * @param name
	...
	/**
	 * Make a note that the parser has finished parsing a non-terminal. This
	 * note is only recorded if the user has requested it.
	 *
	 * @param name
	 *            The name of the non-terminal.
	 */
	...
	/*private */
	public void noteParserInfo(String name) {
