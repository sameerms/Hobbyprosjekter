    ...
    def setlist(self, items_with_help):
        ...
        for (item, help) in list:
            ...
            self.listbox.tag_bind(tag, '<Enter>',
               Command(self.configure, tag, 'yellow', help))
            self.listbox.tag_bind(tag, '<Leave>',
               Command(self.configure, tag, 'white', ''))
            self.listbox.tag_bind(tag, '<Button-1>',
               Command(self.record, tag, counter, item))
            ...
            #   f(event, t, c, text))
            # with class Command as alternative to lambda functions:
