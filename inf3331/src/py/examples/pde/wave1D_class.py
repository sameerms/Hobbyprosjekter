...
class WaveEq1(PrmDictBase):
    def __init__(self, **kwargs):
        ...
        self.x  = zeros(1, Float)  # grid points
        self.up = zeros(1, Float)  # sol. at new time level
    ...
    def solve_at_this_time_step(self):
        ...
        if self.numerical_prm['scheme_coding'] == 'scalar':
            ...
            for i in isequence(start=1, stop=n-1):
                up[i] = - um[i] + 2*u[i] + \
...
# solve same problem as in test_plug_solver
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print """Usage %s test_plug_WaveEq1 1 "'vectorized'" """ % \
              sys.argv[0]
        sys.exit(0)
